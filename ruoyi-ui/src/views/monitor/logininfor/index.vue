<template>
  <div class="app-container">
    ...
    <el-table ref="tables" v-loading="loading" :data="list" @selection-change="handleSelectionChange" :default-sort="defaultSort" @sort-change="handleSortChange">
      ...
      <el-table-column label="Number" align="center" prop="infoId" />
      ...
      <el-table-column label="Operation System"width="150"  align="center" prop="os" />
      <el-table-column label="Status" align="center" prop="status">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_common_status" :value="scope.row.status"/>
        </template>
      </el-table-column>
      <el-table-column label="Result " align="center" prop="msg" :show-overflow-tooltip="true" />
...
export default {
  ...
  methods: {
    ...
    /** Clear按钮Action */
    handleClean() {
      this.$modal.confirm('Confirm clear all login infos ?').then(function() {
    ...
    handleUnlock() {
      ...
      this.$modal.confirm('Confirm unlock user "' + username + '"?').then(function() {
        return unlockLogininfor(username);
      }).then(() => {
        this.$modal.msgSuccess("User " + username + " unlocked");
      }).catch(() => {});
    },
    /** Export按钮Action */
    handleExport() {
